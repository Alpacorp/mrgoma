...
const PromoBanner: FC<PromoBannerProps> = ({ content, className = '', storageKey }) => {
  ...
  return (
    <aside
      ...
      className={`w-full overflow-hidden rounded-lg ${bgColor} ${textColor} ${className}`}
    >
      {/* Attention accent bar */}
      <div className="h-1 w-full bg-gradient-to-r from-lime-400 via-emerald-400 to-lime-400 animate-[pulse_3s_ease-in-out_infinite] opacity-80" />
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="relative flex items-stretch gap-4 py-4 sm:py-5">
          <div className="flex-1 min-w-0 pr-12 sm:pr-0">
            ...
            {description && (
              <p className="mt-1 text-sm sm:text-base opacity-90">{description}</p>
            )}
            {ctaLabel && ctaHref && (
              <div className="mt-3">
                <Link
