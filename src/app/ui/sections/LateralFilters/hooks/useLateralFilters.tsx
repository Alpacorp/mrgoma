...
export const useLateralFilters = () => {
  ...
  useEffect(() => {
    const fetchBounds = async () => {
      try {
        const res = await fetch('/api/tires/ranges');
        if (!res.ok) return;
  ...
  useEffect(() => {
    const newRangeInputs = {
      price: [
        parseInt(
          searchParams.get('minPrice') || rangeBounds.price[0].toString(),
          10,
        ),
        parseInt(
          searchParams.get('maxPrice') || rangeBounds.price[1].toString(),
          10,
        ),
      ],
      treadDepth: [
        parseFloat(
          searchParams.get('minTreadDepth') ||
            rangeBounds.treadDepth[0].toString(),
        ),
        parseFloat(
          searchParams.get('maxTreadDepth') ||
            rangeBounds.treadDepth[1].toString(),
        ),
      ],
      remainingLife: [
        parseInt(
          searchParams.get('minRemainingLife') ||
            rangeBounds.remainingLife[0].toString(),
          10,
        ),
        parseInt(
          searchParams.get('maxRemainingLife') ||
            rangeBounds.remainingLife[1].toString(),
          10,
        ),
      ],
    } as RangeInputs;
  ...
  const resetFilters = useCallback(() => {
    ...
    // Clear filter parameters from URL and reset pagination
    const params = new URLSearchParams(searchParams.toString());
    params.set('page', '1');
