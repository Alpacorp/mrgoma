...
- STRIPE*SECRET_KEY=sk_live*... (or test key sk*test*...)
- NEXT*PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_live*... (optional for future client SDK usage)
...
## About NEXT*PUBLIC* environment variables

In Next.js, variables prefixed with NEXT*PUBLIC* are exposed to the browser at build/runtime and are appropriate for non‑sensitive, UI-related configuration. Keep the following as NEXT*PUBLIC* because they either drive client UI, links, or harmless display values:
...
Server-only variables must NOT be prefixed with NEXT*PUBLIC* (they stay private):
...
If you ever need the same value on both server and client, declare it twice: once without NEXT*PUBLIC* for server-only logic, and once with NEXT*PUBLIC* for client consumption (ensuring it is not sensitive).
