...
- [ ] **T9 — Manual check** _(yours: DevTools + visual)_ · `npm run dev` on a real
  detail URL: main image server-rendered `fetchpriority=high` + **not** `loading=lazy`;
  footer doesn't shift on load; zoom (desktop lens + mobile fullscreen), tread 3D,
  carousel, add-to-cart, sold/stock all work; not-found + fallback image + sold states
  preserved. · check: attributes present, no CLS, all interactions OK.
- [~] **T-DoD — Definition of Done** _(automated gates green; manual + PSI are yours)_
  ...
  ✓ (`/tires/[slug]` now `ƒ Dynamic`). Manual check per **T9** and the AC2/AC3
  authoritative **post-deploy PSI** run (CLS < 0.1, LCP improved, "discoverable" passes)
  are yours to record.
