...
and `websiteJsonLd`'s `publisher` — and the **only** two inline copies of the
organisation in the entire app are in `guides/[slug]/page.tsx`:
...
They are there because that page builds its `Article` node inline rather than
through an emitter. **The duplication exists exactly where the rule was
bypassed**, which makes FR9 the mechanism rather than a style preference.
...
- `buildLocationsJsonLd()`: `'@type': 'AutoPartsStore'` becomes
  ...
  `geo`, `hasMap`, `openingHours`, `areaServed` and `address` were verified store
  by store on 2026-08-04.
...
Article · FAQPage · Question · Answer · Product · Offer
```
...
| AC2 | The two inline copies in the guide page are replaced by `{ '@id': … }` | `structuredData.guard.test.ts`: walks `src/app`, and outside `seo.ts` finds **no** `'@type': 'Organization'` at all. That is the enforceable form — the rule is "only `seo.ts` defines the organisation" |
...
| AC9 | `buildLocationsJsonLd` retyped | `seo.test.ts`: all seven are `['TireShop', 'AutoRepair']`, and `geo`, `hasMap`, `openingHours`, `areaServed` and `address` are byte-identical to before — asserted field by field, since the risk here is touching what was verified in August |
